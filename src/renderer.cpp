...
Renderer::Renderer(size_t width, size_t height)
    : screen_(width, height), zbuffer_(width, height) {}

void Renderer::renderFrame(sf::RenderWindow &window, const World &world,
                           const Camera &Camera) {
...
double Renderer::getInterpolateCoef(size_t left, size_t mid,
                                    size_t right) const {
    assert(left != right && left <= mid && mid <= right &&
           "Bad interpolating coefficient");
...
void Renderer::renderTriangle(const Primitives::Triangle &triangle) {
    ...
    for (size_t y = yMin; y <= yMax; ++y) {
        Primitives::Vertex vLeft = Primitives::Vertex::interpolate(
            vMin, vMax, getInterpolateCoef(yMin, y, yMax));
        Primitives::Vertex vRight =
            (yMid > yMin && y <= yMid)
                ? Primitives::Vertex::interpolate(
                      vMin, vMid, getInterpolateCoef(yMin, y, yMid))
                : Primitives::Vertex::interpolate(
                      vMid, vMax, getInterpolateCoef(yMid, y, yMax));
...
void Renderer::renderLine(const Primitives::Vertex &vLeft,
    ...
    if (xLeft == xRight) {
        updateZBuffer(
            xLeft, y,
            (vLeft.getPosition().z < vRight.getPosition().z ? vLeft : vRight));
    } else {
        for (size_t x = xLeft; x <= xRight; ++x) {
            updateZBuffer(
                x, y,
                Primitives::Vertex::interpolate(
                    vLeft, vRight, getInterpolateCoef(xLeft, x, xRight)));
        }
    }
}

void Renderer::drawFrame(sf::RenderWindow &window) {
    window.draw(screen_.getDrawData().data(), screen_.getDrawData().size(),
                sf::Points);
    zbuffer_.clear();
    screen_.clear();
}
